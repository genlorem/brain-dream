...
{"id":"стабильный-короткий-id","kind":"error|idea","signal":"signal-id или null",
 "hypothesis":"проверяемая гипотеза улучшения",
 "keywords":["конкретная многословная github-фраза", "..."],
 "domainTerms":["термин домена", "..."],
 "negativeTerms":["маркер чужого домена", "..."]}.
...
4. Для каждой гипотезы дай 2–4 КОНКРЕТНЫХ многословных англоязычных запроса.
   Не используй общие однословные запросы вроде dedup, graph или memory.
5. Смешивай стратегии поиска:
   - категория+платформа: "claude code hook", "knowledge graph linking",
     "llm memory dedup", "notion cli";
   - конкурент/alternative: "<название аналога> alternative";
   - стек+домен: "pgvector recall", "embedding dedup".
6. domainTerms — 2–5 слов или коротких терминов, хотя бы один из которых обязан
   встречаться в названии или описании релевантного репозитория.
7. negativeTerms — маркеры чужих доменов, которые надо отсечь, например:
   backup, restic, network, pentest, downloader, anki.
8. JSON обязан быть валидным; id не должны повторяться.
